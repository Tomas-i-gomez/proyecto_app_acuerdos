...
const RamoTable = () => {
  ...
  const filteredRamo = ramos ?
    ramos.filter((ramo) => ramo.name.toLowerCase().includes(searchQuery.toLowerCase())): [];
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50">
              <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">RamoID</th>
          ...
          <tbody>
            {currentRamo.map((ramo) => (
              <tr key={ramo.id} className="border-t">
                ...
                <td className="px-6 py-4 text-sm text-gray-800">{ramo.name}</td>
                <td className="px-6 py-4 text-right">
                  <div className='flex items-center space-x-4'>
                    <button 
                    onClick={() => {
                      const confirmed = window.confirm("¿Estás seguro de eliminar el ramo?");
                      if (confirmed) {
